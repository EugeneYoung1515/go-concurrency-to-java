...
public class Chan<T> implements Iterable<T> {
    ...
    boolean send(T data, boolean nonblock) {
        while (true) {
            ...
            try {
                ...
                //原理
                //如果消费者叫醒的 这边判断是已经从等待队列中移除了
                //如果是关闭叫醒的 关闭不会把等待者从队列中移除 这边是还能再移除 就会丢出异常
    ...
    Result<T> receive(boolean nonblock) {
        while (true) {
            ...
            //消费者处于receive的阻塞时
            //channel被关闭 最后会运行到下一个循环 之后可能运行到这里
            ...
            ThreadContext context = null;
            try {
                context = ThreadContext.get();//这里是加入到等待者队列
                if (Config.DEBUG_CHECK_STATE) {
                    context.ensureHasNoChan();
                }
                ...
                boolean exist = mReceivers.remove(context);//这里移除
                if (Config.DEBUG_PRINT) {
